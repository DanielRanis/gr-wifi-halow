...
c = ieee802_11.constellation_256qam()
a = np.array(c.points())
p = np.average(np.abs(a)**2)
level = (1.0/170)**.5

print "256-QAM, average power: {0:.4f}".format(p)
...
if any(rx != data):
    print "256-QAM: data does not match."
else:
    print "256-QAM: points decoded successfully."

plt.scatter(a.real, a.imag)
plt.scatter(noisy_const.real, noisy_const.imag, marker='x')
for d, a, b in zip(rx, rx_const, noisy_const):
    plt.plot([a.real, b.real], [a.imag, b.imag], color=mpl.cm.hsv(d/256.0))
